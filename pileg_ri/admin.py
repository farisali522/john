...
class DapilAdmin(ImportExportModelAdmin):
    ...
    # list_display dibuat dinamis
    search_fields = ('nama',)
    list_filter = ('kokab',)
    filter_horizontal = ('kokab',)
    ordering = ('id',)

    def get_queryset(self, request):
        qs = super().get_queryset(request)
        return qs.prefetch_related('kokab__kecamatan_set__rekapsuara__detail_suara')

    @admin.display(description='Cakupan Wilayah (Kota/Kab)')
    def get_kokab(self, obj):
        return ", ".join([k.nama for k in obj.kokab.all()])

    def get_suara_partai_dict(self, dapil_obj):
        if hasattr(dapil_obj, '_suara_partai_dict'):
            return dapil_obj._suara_partai_dict

        # HANYA kumpulkan suara dari partai yang LOLOS PT 4%
        partai_dict = {p.id: 0 for p in Partai.objects.filter(is_lolos_pt=True)}

        for kokab in dapil_obj.kokab.all():
            for kec in kokab.kecamatan_set.all():
                if hasattr(kec, 'rekapsuara'):
                    for detail in kec.rekapsuara.detail_suara.all():
                        # Jika partai tidak lolos PT, suaranya otomatis diabaikan dari perhitungan
    ...
    def get_global_kursi_partai(self):
        ...
        from master.sainte_lague import hitung_sainte_lague
        global_kursi = {}
        for dapil in Dapil.objects.prefetch_related('kokab__kecamatan_set__rekapsuara__detail_suara'):
    ...
    def get_partai_header(self, partai):
        ...
        # Style abu-abu jika gagal PT
        opacity = "1" if partai.is_lolos_pt else "0.4"
        bg_logo = "#f3f4f6" if partai.is_lolos_pt else "#e5e7eb"

        logo_html = format_html(
            '<span style="display:inline-flex;align-items:center;'
            'justify-content:center;width:28px;height:28px;border-radius:4px;'
            'background:{};border:1px solid #d1d5db;color:#9ca3af;'
            'font-size:12px;opacity:{};">{}</span>',
            bg_logo, opacity, partai.no_urut
        )

        if partai.logo_url:
            logo_html = format_html(
                '<img src="{}" alt="{}" style="width:28px;height:28px;'
                'object-fit:contain;border-radius:4px;border:1px solid #d1d5db;opacity:{};">',
                partai.logo_url,
                partai.nama,
                opacity
            )
            
        if partai.is_lolos_pt:
            badge_html = format_html('<span style="color:#059669; font-weight:bold; font-size:12px; margin-top:2px; padding:0 4px; background:#dcfce7; border-radius:4px;">{} Kursi</span>', total_kursi)
        else:
            badge_html = format_html('<span style="color:#dc2626; font-weight:bold; font-size:10px; margin-top:2px; padding:0 4px; background:#fee2e2; border-radius:4px;">Gagal PT</span>')

        return format_html(
            '<span style="display:inline-flex;flex-direction:column;'
            'align-items:center;gap:2px;min-width:52px;line-height:1.1;">'
            '{}<strong style="opacity:{};">#{}</strong><small style="font-weight:400;opacity:{};">{}</small>'
            '{}'
            '</span>',
            logo_html,
            opacity, partai.no_urut,
            opacity, partai.nama,
            badge_html
        )
    ...
    def make_kursi_partai_method(self, partai):
        def kursi_partai(obj):
            ...
            if jumlah_kursi > 0:
                ...
                if not partai.is_lolos_pt:
                    return format_html('<div style="text-align:center; color:#ef4444; font-size:11px; opacity:0.6;">X</div>')
