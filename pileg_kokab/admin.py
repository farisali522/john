...
class DapilAdmin(ImportExportModelAdmin):
    ...
    def get_kecamatan_readonly(self, obj):
        if not obj or not obj.pk:
            return '-'
        names = [k.nama for k in obj.kecamatan.all()]
        return ", ".join(names) if names else '-'

    def get_fields(self, request, obj=None):
        fields = ['nama', 'jumlah_kursi', 'kokab']
        if not self.has_change_permission(request, obj):
            fields.append('get_kecamatan_readonly')
        else:
            fields.append('kecamatan')
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if 'get_kecamatan_readonly' not in readonly:
            readonly.append('get_kecamatan_readonly')
...
class RekapKursiKokabAdmin(admin.ModelAdmin):
    ...
    def make_readonly_total_kursi_partai_method(self, partai):
        def kursi_partai_readonly(self, obj):
            if not obj or not obj.pk:
                kursi = 0
            else:
        ...
        kursi_partai_readonly.short_description = f'Partai #{partai.no_urut}'
        kursi_partai_readonly.__name__ = f'readonly_total_kursi_partai_{partai.pk}'
        return kursi_partai_readonly

    def get_fields(self, request, obj=None):
        fields = ['kode', 'nama']
        for partai in Partai.objects.order_by('no_urut'):
            method_name = f'readonly_total_kursi_partai_{partai.pk}'
            if not hasattr(self.__class__, method_name):
                setattr(self.__class__, method_name, self.make_readonly_total_kursi_partai_method(partai))
            fields.append(method_name)
        return fields
